...
export class SchemeLoader implements ISchemeLoader {
    ...
    for (const str in structure) {
      ...
      for (const method in routes) {
        ...
        if (details) {
          ...
          if (route) {
            throw new Error(
              `Route path "${str}" with method "${method.toUpperCase()}" has already exist`
            );
          } else {
  ...
  >(service: S, domain: D, method: M, route: R): NAbstractSchemeLoader.Route {
    ...
    if (!rStorage) {
      throw new Error(
        `Handler for route "${route}" or method "${method}" in domain "${domain}" from service "${service}" not exist.`
      );
    }
  ...
  public setEvent(domain: string, structure: EmitterStructure): void {
    ...
    for (const name in structure) {
      ...
      if (!handler) {
        ...
        throw new Error(
          `Event "${name}" from domain "${domain}" belong to alias "${belongTo}" has already exist.`
        );
      }
    }
  }
  ...
  >(service: S, domain: D, belongTo: B, event: E): NAbstractSchemeLoader.Event {
    ...
    if (!handler) {
      throw new Error(
        `Handler for event "${event}" from domain "${domain}" belong to alias "${belongTo}" in service "${service}" not found.`
      );
    }
  ...
  public setSubject(domain: string, structure: BrokerStructure): void {
    ...
    for (const name in structure) {
      ...
      if (!handler) {
        ...
        throw new Error(
          `Subject "${name}" from domain "${domain}" belong to alias "${belongTo}" has already exist.`
        );
      }
    }
  }

  public getSubjects<S extends string = string, D extends string = string>(
    service: S,
    domain: D
  ): NAbstractSchemeLoader.Subjects {
  ...
  >(service: SER, domain: D, belongTo: B, subject: SUB): NAbstractSchemeLoader.Subject {
    ...
    if (!handler) {
      throw new Error(
        `Subject "${subject}" from domain "${domain}" belong to alias "${belongTo}" not exist.`
      );
    }
  ...
  public setDictionaries(
    ...
    if (Array.isArray(dictionaries)) {
      dictionaries.forEach((d) => this._setDictionary(domain, d.language, d.dictionary));
    } else {
      this._setDictionary(domain, dictionaries.language, dictionaries.dictionary);
    }
  }
  ...
  private _setDictionary(
    domain: string,
    language: string,
    dictionary: NAbstractSchemeLoader.Dictionary
  ): void {
    const dStorage = this._domains.get(domain);
    if (!dStorage) {
      this.setDomain(domain);
      this._setDictionary(domain, language, dictionary);
      return;
    }
    ...
    if (!dict) {
      ...
      throw new Error(
        `Dictionary with language "${language}" in domain "${domain}" has already exist.`
      );
    }
  }
  ...
  >(service: S, domain: D, language: L): NAbstractSchemeLoader.Dictionary {
    ...
    if (!dictionary) {
      throw new Error(
        `Dictionary with language "${language}" in domain "${domain}" from service "${service}" not exist.`
      );
    }
  ...
  public setValidator<H extends string = string>(
    domain: string,
  ...
  >(service: S, domains: D, name: V, agents: A): R {
    ...
    if (!validator) {
      throw new Error(
        `Validator "${name}" in domain ${domains} from service "${service}" not exist.`
      );
    }
